...
import { ArrowUpRight, ChevronDown, MapPin, Menu, Search } from "lucide-react";
...
function Header() {
  return (
    <header className="site-header">
      ...
      <div className="header-actions"><a className="search-link" href="/knowledge"><Search size={18} /><span>Search</span></a><a className="contact-button" href="/contact">Partner with APIN <ArrowUpRight size={16} /></a><button className="menu-button" aria-label="Open menu"><Menu /></button></div>
    </header>
  );
}
    ...
    <main>
      ...
      <section className="hero">
        ...
        <div className="hero-copy">
          <p className="eyebrow">APIN Public Health Initiatives <span>Est. 1999</span></p>
      ...
      <section className="capabilities section-wrap">
        ...
        <div className="capability-grid">{capabilities.map((item) => <a className="capability" href="/programmes" key={item.number}><span className="cap-number">{item.number}</span><h3>{item.title}</h3><p>{item.description}</p><span className="round-arrow"><ArrowUpRight size={17} /></span></a>)}</div>
        <a className="underlined-link" href="/programmes">View all capabilities <ArrowUpRight size={16} /></a>
      </section>
      ...
      <section className="impact-teaser section-wrap"><div className="impact-number">01 <span>/ 03</span></div><div><p className="eyebrow">Our impact</p><h2>Evidence that moves<br /><em>health forward.</em></h2><p className="body-copy">We make the work visible: the results, the lessons and the people behind every number.</p><a className="red-button" href="/impact">Visit the impact hub <ArrowUpRight size={17} /></a></div><div className="quote-block"><span className="quote-mark">“</span><blockquote>We are not just delivering projects. We are building the capacity to deliver health for generations.</blockquote><cite>— Dr. Obinna Onyekwena, Chief Executive Officer</cite></div></section>

      <section className="knowledge-strip"><div className="section-wrap knowledge-layout"><div><p className="eyebrow">From the knowledge centre</p><h2>Ideas into<br /><em>action.</em></h2></div><article className="featured-resource"><span className="resource-type">Featured publication · 2024</span><h3>Strengthening primary health care: lessons from a quarter century of partnership</h3><a className="underlined-link" href="/knowledge">Browse the knowledge centre <ArrowUpRight size={16} /></a></article></div></section>

      <footer className="site-footer"><div className="footer-top"><a className="brand footer-brand" href="/"><Mark /></a><div><p className="footer-label">Stay connected</p><p className="footer-copy">Updates on the work shaping a healthier Nigeria.</p></div><a className="red-button" href="/contact">Contact APIN <ArrowUpRight size={17} /></a></div><div className="footer-bottom"><span>© 2024 APIN Public Health Initiatives</span><span>Abuja · Lagos · Across Nigeria</span><a href="/accountability">Accountability & governance <ArrowUpRight size={14} /></a></div></footer>
    </main>
  );
}